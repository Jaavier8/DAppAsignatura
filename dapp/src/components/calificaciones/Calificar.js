...
const Calificar2 = (props) => {
    ...
    return (<article className="AppMisDatos">
        <SoyProfesor>
            {(props.studentDirection !== "" && props.evIndex !== "") ?
                <>
                    <h3>Modificar calificación</h3>

                    <h5>Dirección alumno: {props.studentDirection}</h5>
                    <h5>Evaluación: {props.evIndex}</h5>
                    ...
                    <form>
                        ...
                        <button key="submit" className="pure-button" type="button"
                            onClick={ev => {
                                ev.preventDefault();
                                const stackId = drizzle.contracts.Asignatura.methods.califica.cacheSend(props.studentDirection, props.evIndex, tipo, calificacion);
